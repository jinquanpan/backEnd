  ...
  <div>
    <el-card>
      ...
      >
        ...
        <el-table-column prop="fileReviewer" label="审批人" width="150" />
        <el-table-column
label="操作"
style="text-align: left"
>
          <template slot-scope="{row}">
            <el-button type="primary" size="mini" @click="handleReview(row)">批准</el-button>
...
<script>
import { workbenchListApproving, workbenchApprove, invalidFile } from '@/api/workbench-doc-control'
...
export default {
  ...
  methods: {
    ...
    // 批准
    handleReview(row) {
      this.$myConfirm(row.fileName + ' 通过批准', (done) => {
    ...
    handleInvalidFile(row) {
      this.$myConfirm(row.fileName + '，且不可逆', (done) => {
