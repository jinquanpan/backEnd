  ...
  <el-row>
    ...
    <el-card>
      <el-table
              ...
              style="width: 100%"
      >
        <el-table-column prop="folderName" label="文件夹名称" width="300">
          <template slot-scope="{row}">
            <FileNameContainer :row="row" @folder-click="$_inserFolder(row)" @menu-click="showFileDetail(row)" />
          </template>
        </el-table-column>
        <el-table-column prop="fileNo" label="记录编号" width="150" />
        <el-table-column prop="fileName" label="记录名称" width="150" />
...
import {
  listExamine,
  docAuditManageDeleteFolder,
  docAuditManageDeleteFile,
  docAuditManageSaveFolder,
  docAuditManagerenameFolder
} from '@/api/doc-audit-internal'
export default {
  components: {
    VersionList,
    FileDetail,
  ...
  created() {
    this.getList()
    // this.obtainFileTypes()
    // this.obtainFileStatus()
  },
  methods: {
    // 列表
    getList() {
    ...
    // obtainFileTypes(){
    //   fileType().then(res=>{
    //     this.obtain.fileType=res
    //     // console.log(this.obtain)
    //     // console.log(res)
    //   })
    // },
    // obtainFileStatus(){
    //   fileStatus().then(res=>{
    //     this.obtain.fileStatus=res
    //     console.log(res)
    //   })
    // },
    // 新增或编辑文件夹
      ...
      this.$myPrompt({
        ...
        success: (value) => {
          ...
          if (row) {
            ...
            http = docAuditManagerenameFolder
          } else {
            http = docAuditManageSaveFolder
          }
          http(form).then(() => {
            this.getList()
    ...
    handleDelFile(row) {
      this.$delConfirm(row.fileName, (done) => {
        docAuditManageDeleteFile(row.docAuditInternalId).then(() => {
          done()
          this.getList()
        })
      })
    ...
    handleDelFolder(row) {
      this.$delConfirm(row.folderName, (done) => {
        docAuditManageDeleteFolder({ folderId: row.folderId }).then(() => {
          done()
          this.getList()
        })
      })
    ...
    // 显示新建文件弹框
    handleAddFile(row) {
      if (typeof row !== 'undefined' && !row.docAuditInternalId) {
        return this.$message.warning('参数错误')
      }
      this.$refs.fileForm.show(row)
    },
    showFileDetail(row) {
      if (!row.docAuditInternalId) {
        return this.$message.warning('参数错误')
      }
      this.$refs.fileDetail.show(row.docAuditInternalId)
    },
    // 获取版本列表
    getVersions(row) {
      if (!row.fileNo) {
